...
const MonitorTab: React.FC = () => {
  ...
  const handleStockAnalysis = async () => {
    ...
    try {
      ...
      const bid = parseFloat(String(depth.best_bid_price || quote.best_bid_price || 0));
      const ask = parseFloat(String(depth.best_offer_price || quote.best_offer_price || 0));
      const mid = (bid + ask) / 2;
      const spread = mid > 0 ? ((ask - bid) / mid) * 100 : null;
      
      setStockMetrics({
        spread_pct: spread,
        depth_ratio: (parseFloat(String(depth.best_bid_quantity || 0)) + 1) / (parseFloat(String(depth.best_offer_quantity || 0)) + 1),
        vol_ratio: null,
        regime: 'NEUTRAL',
        execution_style: spread && spread < 0.15 ? 'OK FOR MARKET' : 'LIMIT ONLY',
        bid, ask, 
        bidQty: parseFloat(String(depth.best_bid_quantity || 0)), 
        askQty: parseFloat(String(depth.best_offer_quantity || 0)),
        avg_vol_20d: null
      });
